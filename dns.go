...
func (d *DNSServer) readQuery(m *dns.Msg, remoteAddr net.Addr) {
	...
	if authoritative {
		if m.MsgHdr.Rcode == dns.RcodeNameError || (m.MsgHdr.Rcode == dns.RcodeSuccess && len(m.Answer) == 0) {
...
func (d *DNSServer) hasTxtForDomain(q dns.Question) bool {
	...
	txt, err := d.DB.GetTXTForDomain(subdomain)
...
func (d *DNSServer) getRecord(q dns.Question) ([]dns.RR, error) {
	var rr []dns.RR
	var cnames []dns.RR
	domain, ok := d.getRecordByDomainName(q.Name)
	if !ok {
...
func (d *DNSServer) answer(q dns.Question, remoteAddr net.Addr) ([]dns.RR, int, bool, error) {
	...
	log.WithFields(log.Fields{"qtype": dns.TypeToString[q.Qtype], "remote_addr": remoteAddr, "domain": q.Name, "rcode": dns.RcodeToString[rcode]}).Debug("Answering question for domain")
